...
def get_dominant_month(conn, company_id: str, table: str) -> str | None:
    """Returns None if the company has zero rows in this table — known
    case: SBIN has zero balancesheet rows (Sprint 1 Day 6, genuine
    source gap, not a bug)."""
    years = [
        r[0]
...
def main() -> None:
    ...
    for cid in company_ids:
        ...
        if dom_month is None:
            skipped_no_bs.append(cid)
            continue
    ...
    print(f"Skipped (no balancesheet rows, e.g. SBIN): {skipped_no_bs}")
